...
#if MPI_VERSION >= 4
template <send_buffer SBuf, recv_buffer_v_c RBuf, convertible_to_mpi_handle<MPI_Comm> Comm = MPI_Comm>
void allgatherv_c(SBuf&& sbuf, RBuf&& rbuf, Comm const& comm = MPI_COMM_WORLD) {
    int err = MPI_Allgatherv_c(
        ptr(sbuf),
        count(sbuf),
        type(sbuf),
        ptr(rbuf),
