...
///   1. Include thrust_buffer_traits.hpp — device_vector works directly, no piping needed.
...
int main(int argc, char* argv[]) {
    ...
    // Example 2 (buffer_traits): deferred-size receive — pipe through views::resize.
    if (world.rank() == 0) {
        thrust::device_vector<int> d_send(N);
    ...
    // Example 3 (view adaptor): explicit per-call opt-in via views::thrust::device_ptr.
    if (world.rank() == 0) {
        thrust::device_vector<int> d_send(N);
        thrust::sequence(d_send.begin(), d_send.end(), 300);
        kamping::v2::send(d_send | kamping::v2::views::thrust::device_ptr, 1, 0, world);
    } else {
        thrust::device_vector<int> d_recv(N);
        kamping::v2::recv(d_recv | kamping::v2::views::thrust::device_ptr, 0, 0, world);
