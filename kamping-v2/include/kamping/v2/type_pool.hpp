...
struct with_pool_fn {
    ...
    ///
    /// Uses `flat_element_t`, not `payload_element_t`: `R` is always taken to be an already-flat
    ...
    /// flat_element_t's doc comment in payload.hpp for why that distinction matters.
    template <typename R>
        requires requires { typename kamping::v2::flat_element_t<std::remove_cvref_t<R>>; }
                 && kamping::types::has_static_type_v<kamping::v2::flat_element_t<std::remove_cvref_t<R>>>
    auto operator()(R&& r, type_pool const& pool) {
        using elem_t = kamping::v2::flat_element_t<std::remove_cvref_t<R>>;
    ...
    template <typename R, typename Env>
        requires has_pool<std::remove_cvref_t<Env>>
                 && requires { typename kamping::v2::flat_element_t<std::remove_cvref_t<R>>; }
                 && kamping::types::has_static_type_v<kamping::v2::flat_element_t<std::remove_cvref_t<R>>>
...
struct with_auto_pool_fn {
    ...
    /// Works with standard ranges and any buffer type exposing a public `value_type` member.
    ///
    /// Uses `flat_element_t`, not `payload_element_t` — see @ref with_pool_fn.
    template <typename R>
        requires requires { typename kamping::v2::flat_element_t<std::remove_cvref_t<R>>; }
                 && kamping::types::has_static_type_v<kamping::v2::flat_element_t<std::remove_cvref_t<R>>>
